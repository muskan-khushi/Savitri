...
**Art track — done.** All 12 pieces (A1–A5, B1–B5, C1–C2) have been drawn, converted to web-optimized WebP, and organized into `public/art/` per the naming convention in Section 9, with untouched originals archived separately in `design-source/raw/`. A few notes worth carrying forward:
- **A5 (the hero)** is 1672×941px — solid for the web, but the lowest-margin file of the set relative to very large displays. Worth revisiting first if the art ever gets a second pass at higher resolution; nothing was artificially upscaled to avoid faking detail that isn't there.
- **C2 (empty state)** over-delivered: it's not just a static seed image, it's a seed plus a 3-frame seed→sprout progression in one composition — usable as-is, or as a head start on an actual animated loading sequence later.
- **B5 (second income)** landed as a solar panel with a sprout beside its base rather than emerging from a corner as originally described in the brief — a minor deviation that still reads correctly, kept rather than sent back for a redraw.

The remaining work is entirely on the code side now: Phase 1 of the Build Plan (real weather → real irrigation number) is the natural next step, followed by wiring the now-finished `public/art/` assets into the `components/illustration/` components (`MythBeat.tsx`, `MotifIcon.tsx`, `EmptyState.tsx`) named in Section 9.